    ...
    with upload_container:
        ...
        if uploaded_file:
            ...
            with st.spinner("Procesando datos..."):
                ...
                # Orden de periodos para gráficos de caja
                period_order: List[str]
                if freq_final == "Q":
                    period_order = ["Q1", "Q2", "Q3", "Q4"]
                elif freq_final == "M":
                    period_order = [calendar.month_abbr[i] for i in range(1, 13)]
                elif freq_final == "W":
                    period_order = sorted(
                        processed_df["Periodo"].unique().tolist(),
                        key=lambda x: int(x.split(" ")[1]) if isinstance(x, str) and " " in x else 0,
                    )
                elif freq_final == "A":
                    period_order = sorted(processed_df["Periodo"].unique().tolist())
                else:
                    period_order = ["Lun", "Mar", "Mié", "Jue", "Vie", "Sáb", "Dom"]
                ...
                for cat in seleccionadas:
                    if cat not in per_cat:
                        st.warning(f"No se pudo construir la descomposición para '{cat}'.")
                        continue
                    ...
                    crecimiento_df = cat_df[["Fecha", "g"]].dropna()
                    crecimiento_chart = (
                        alt.Chart(crecimiento_df)
                        .mark_line(point=True, color=palette["growth"])
                        .encode(
                            x=alt.X("Fecha:T", title="Fecha"),
                            y=alt.Y("g:Q", title="Crecimiento (%)"),
                        )
                    ...
                    tendencia_estacionalidad = pd.DataFrame(
                        {
                            ...
                            "Estacionalidad": per_cat[cat]["y_season"].values,
                        }
                    )
                    tendencia_long = tendencia_estacionalidad.melt(
                        "Fecha", var_name="Componente", value_name="Valor"
                    )
                    tendencia_chart = (
                        alt.Chart(tendencia_long)
                        .mark_line()
                        .encode(
                            x=alt.X("Fecha:T", title="Fecha"),
                            ...
                            color=alt.Color(
                                "Componente:N",
                                scale=alt.Scale(range=[palette["trend"], palette["season"]]),
                                legend=alt.Legend(orient="bottom"),
                            ),
                        )
                        .properties(height=220)
                    ...
                    estacionalidad_df = tendencia_estacionalidad.copy()
                    estacionalidad_df["Periodo"] = cat_df["Periodo"].values
                    box_chart = (
                        alt.Chart(estacionalidad_df)
                        .mark_boxplot(color=palette["season"], opacity=0.7)
                        .encode(
                            x=alt.X("Periodo:N", sort=period_order, title="Periodo"),
                            y=alt.Y("Estacionalidad:Q", title="Estacionalidad"),
                        )
                        .properties(height=220)
                        .configure_axis(grid=False)
                    )
                    ...
                    with extra_col1:
                        with st.container(border=True):
                            ...
                            st.altair_chart(crecimiento_chart, use_container_width=True)
                    with extra_col2:
                        with st.container(border=True):
                            st.markdown("**Tendencia vs. estacionalidad**")
                            st.altair_chart(tendencia_chart, use_container_width=True)

                    with st.container(border=True):
                        st.markdown("**Distribución estacional por periodo**")
